...
void SSD1306Device::ssd1306_string_font6x8_eeprom(char *s) {
	while (*s) {
		ssd1306_char_font6x8_eeprom(*s++);
	}
}

void SSD1306Device::ssd1306_char_font6x8_eeprom(char ch) {
	uint16_t c = (ch - 32) * 5;
	ssd1306_send_data_start();
  SSD1306.ssd1306_send_byte(0x00);
  for (byte i = 5; i; i--) {
    ssd1306_send_byte(EEPROM.read(c++));
  }
	ssd1306_send_data_stop();
}

void SSD1306Device::ssd1306_int_font6x8_eeprom(int num) {
  char str[5];
  utoa(num, str, DEC);
  ssd1306_string_font6x8_eeprom(str);
}

void SSD1306Device::ssd1306_float_font6x8_eeprom(double number, uint8_t digits)
{
  if (number < 0.0)
  {
    ssd1306_string_font6x8_eeprom("-");
    number = -number;
  }
  ...
  // n += int_part;
  ssd1306_int_font6x8_eeprom(int_part);
  ...
  while (digits-- > 0)
  {
    ...
    // n += toPrint;
    ssd1306_int_font6x8_eeprom(toPrint);
    remainder -= toPrint;
